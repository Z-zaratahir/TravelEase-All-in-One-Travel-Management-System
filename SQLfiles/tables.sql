...
select * from [User]
ALTER TABLE [User]
ADD IsAccepted INT NOT NULL DEFAULT 0;
...
CREATE TABLE WishList (
    ...
    TravelerID INT unique FOREIGN KEY REFERENCES Traveler(TravelerID)
);
...
ALTER TABLE WishList_Contains
ADD CONSTRAINT UC_WishList_Trip UNIQUE (WishListID, TripID);

select * from WishList_Contains
-- Alter the table to make WishListItemID an identity column
-- Drop the primary key constraint
ALTER TABLE WishList_Contains
DROP CONSTRAINT PK__WishList__DAC208291C7AE67D;

ALTER TABLE WishList_Contains
DROP COLUMN WishListItemID;

-- Recreate the column with the identity property
ALTER TABLE WishList_Contains
ADD WishListItemID INT IDENTITY(1,1) PRIMARY KEY;
 ...
 WHERE 
     ba.TravelerID = 7 AND t.TripID = 44
	 select * from BookingAttempt where TravelerID=7
CREATE TABLE Hotel (
...
drop table Review
ALTER TABLE Review
ADD CONSTRAINT UC_TripID_TravelerID UNIQUE (TripID, TravelerID);
...
WHERE 
    w.WishListID = (SELECT WishListID FROM WishList WHERE TravelerID = 4);
...
VALUES
( 5, '2025-04-01', 'Amazing experience!', 2, 3, 1, 1),
( 4, '2025-04-02', 'Great trip but could improve the accommodations.', 6, 4, 2, 5),
( 5, '2025-04-03', 'The best holiday ever!', 10, 7, 3, 8);
INSERT INTO Review ( Rating, ReviewDate, Comment, OperatorID, TravelerID, TripID, AdminID)
VALUES
(3, '2025-04-04', 'Good trip, but food could be better.', 12, 9, 4, 11),
(4, '2025-04-05', 'Enjoyed the journey, would go again!', 16, 13, 5, 15),
( 5, '2025-04-06', 'Wonderful trip with a fantastic guide.', 20, 14, 6, 18),
( 4, '2025-04-07', 'Beautiful destination, but very crowded.', 22, 17, 7, 23),
( 3, '2025-04-08', 'Decent trip, but the transport was uncomfortable.', 26, 19, 8, 28),
( 5, '2025-04-09', 'Had a blast, everything was perfect!', 30, 21, 9, 31),
( 4, '2025-04-10', 'Great experience, but needs better planning.', 34, 24, 10, 35),
( 5, '2025-04-11', 'Fantastic, I would recommend this trip to everyone!', 37, 25, 11, 41),
( 4, '2025-04-12', 'Good overall, but the weather could have been better.', 40, 27, 12, 45),
( 3, '2025-04-13', 'Could have been better organized.', 44, 29, 13, 1),
( 5, '2025-04-14', 'Excellent trip with amazing views.', 47, 32, 14, 5),
( 4, '2025-04-15', 'The trip was great, but the accommodation was not up to expectations.', 50, 33, 15, 8),
( 3, '2025-04-16', 'Average experience, not what I expected.', 2, 36, 16, 11),
( 4, '2025-04-17', 'Nice, but too much time spent traveling.', 6, 38, 17, 15),
( 5, '2025-04-18', 'Amazing guide and itinerary!', 10, 39, 18, 18),
( 3, '2025-04-19', 'Not a bad trip, but it was very rushed.', 12, 42, 19, 23),
( 4, '2025-04-20', 'Good, but some activities were overpriced.', 16, 43, 20, 28),
( 5, '2025-04-21', 'Everything was perfect, will do again!', 20, 46, 21, 31),
( 4, '2025-04-22', 'Lovely trip, but the meals were not great.', 22, 48, 22, 35),
( 3, '2025-04-23', 'It was okay, but it didn’t meet my expectations.', 26, 49, 23, 41),
( 5, '2025-04-24', 'One of the best experiences of my life!', 30, 51, 24, 45),
( 4, '2025-04-25', 'Nice experience, could have been better organized.', 34, 52, 25, 1),
( 5, '2025-04-26', 'Amazing views and great guides!', 37, 53, 26, 5),
( 3, '2025-04-27', 'A decent trip but nothing extraordinary.', 40, 54, 27, 8),
( 4, '2025-04-28', 'Good trip but the schedule was a bit tight.', 44, 55, 28, 11),
( 5, '2025-04-29', 'A fantastic journey with beautiful experiences.', 47, 56, 29, 15),
( 3, '2025-04-30', 'Too crowded for my liking.', 50, 57, 30, 18)

select * from Traveler
select * from BookingAttempt
select * from Booking
select * from [User]
where UserID=7
...
SELECT * FROM TourCategories
SELECT * FROM TourCategoryActivities
SELECT * FROM Payment
SELECT * FROM Booking
SELECT * FROM BookingAttempt
SELECT * FROM Trip
...
ALTER TABLE Trip
DROP CONSTRAINT FK__Trip__CategoryID__6477ECF3; 

ALTER TABLE TourCategoryActivities
DROP CONSTRAINT FK_Trip_TourCategory;  -- Drop the existing constraint
 -- Replace 'YourTableName' with the name of your table

ALTER TABLE Trip
ADD CONSTRAINT FK_Trip_TourCategory
FOREIGN KEY (CategoryID)
REFERENCES TourCategories (CategoryID)
ON DELETE SET NULL;  -- Set the CategoryID to NULL when the category is deleted

DELETE FROM TourCategoryActivities
WHERE CategoryID NOT IN (SELECT CategoryID FROM TourCategories);

ALTER TABLE TourCategoryActivities
ADD CONSTRAINT FK_TourCategoryActivities_TourCategory
FOREIGN KEY (CategoryID)
REFERENCES TourCategories (CategoryID)
ON DELETE SET NULL;
...
ALTER TABLE Booking
ADD IsConfirmed BIT DEFAULT 0;

------------------Traveler Views
CREATE VIEW view_TotalBookings AS
SELECT 
    COUNT(CASE WHEN b.IsConfirmed = 1 THEN 1 END) AS NumberOfConfirmedBookings,
    COUNT(CASE WHEN b.IsConfirmed = 0 THEN 1 END) AS NumberOfNotConfirmedBookings,
    COUNT(CASE WHEN b.IsConfirmed IS NULL THEN 1 END) AS NumberOfPendingBookings
FROM 
    Booking b;


------------------------------------------------------------------------------------------------
CREATE VIEW view_RevenueByCategory AS
SELECT 
    t.TripID,
    c.CategoryName AS TripCategory,
    CASE
        WHEN b.NumberOfPeople = 1 THEN 'Solo'
        ELSE 'Group'
    END AS Capacity,
    CASE 
        WHEN DATEDIFF(DAY, t.DepartureDate, t.ReturnDate) = 1 THEN '1-Day'
        WHEN DATEDIFF(DAY, t.DepartureDate, t.ReturnDate) = 4 THEN '4-Day'
        ELSE 'Other'
    END AS Duration,
...
Join
    Booking b ON b.AttemptID = ba.AttemptID
JOIN 
    TourCategories c ON c.CategoryID = t.CategoryID
WHERE 
    b.IsConfirmed = 1  -- Only completed bookings
GROUP BY 
    t.TripID, c.CategoryName, 
    CASE WHEN b.NumberOfPeople = 1 THEN 'Solo' ELSE 'Group' END,
    CASE WHEN DATEDIFF(DAY, t.DepartureDate, t.ReturnDate) = 1 THEN '1-Day'
         WHEN DATEDIFF(DAY, t.DepartureDate, t.ReturnDate) = 4 THEN '4-Day' 
         ELSE 'Other' END
----------------------------------------------------------------------------------------------------
CREATE VIEW view_CancellationRate AS
SELECT 
    t.TripID,
    c.CategoryName AS TripCategory,
    COUNT(b.BookingID) AS TotalBookings,
    COUNT(CASE WHEN b.IsConfirmed = 0 OR ba.IsCompleted=0 THEN 1 END) AS CanceledBookings,
    (COUNT(CASE WHEN b.IsConfirmed = 0 OR ba.IsCompleted=0 THEN 1 END) * 100.0 / COUNT(b.BookingID)) AS CancellationRate
FROM 
    Trip t
JOIN 
	BookingAttempt ba ON ba.TripID=t.TripID
...
GROUP BY 
    t.TripID, c.CategoryName
----------------------------------------------------------------------------------------------------------------
CREATE VIEW view_PeakBookingPeriods AS
SELECT
    MONTH(b.Date) AS BookingMonth,
    COUNT(b.BookingID) AS TotalBookings
FROM
    Booking b
GROUP BY
	...
	CREATE VIEW view_AvgBookingValue AS
SELECT
...
FROM
	...
	-----------------------------------------------------------------
CREATE VIEW view_AgeNationalityDistribution AS
SELECT
    u.State,
    CASE
        WHEN DATEDIFF(YEAR, u.DOB, GETDATE()) BETWEEN 18 AND 25 THEN '18-25'
        WHEN DATEDIFF(YEAR, u.DOB, GETDATE()) BETWEEN 26 AND 35 THEN '26-35'
        WHEN DATEDIFF(YEAR, u.DOB, GETDATE()) BETWEEN 36 AND 45 THEN '36-45'
        WHEN DATEDIFF(YEAR, u.DOB, GETDATE()) BETWEEN 46 AND 60 THEN '46-60'
        ELSE '60+'
    END AS AgeGroup,
    COUNT(u.Registration) AS UserCount
FROM
    Person u
GROUP BY
    u.State,
    ...
    END
	-----------------------------------------------------------------
	CREATE VIEW view_PreferredTripTypesDestinations AS
SELECT
    t.CategoryID,
    tc.CategoryName,
    td.Destination,
    COUNT(t.TripID) AS TotalBookings
FROM
    Trip t
JOIN
    TourCategories tc ON t.CategoryID = tc.CategoryID
JOIN
    TripDestinations td ON t.TripID = td.TripID
GROUP BY
    t.CategoryID, tc.CategoryName, td.Destination

	-----------------------------------------------------------------
	CREATE VIEW view_AvgSpendingHabits AS
SELECT
    b.TravelerID,
    AVG(bb.TotalAmount) AS AvgSpending
FROM
    BookingAttempt b
Join
Booking bb on bb.AttemptID=b.AttemptID
GROUP BY
    b.TravelerID
	-----------------------------------------------------------------
	--77777777777
	CREATE VIEW view_NewUserRegistrations AS
SELECT
    MONTH(u.RDate) AS RegistrationMonth,
    YEAR(u.RDate) AS RegistrationYear,
    COUNT(DISTINCT u.Registration) AS NewUsers,
    CASE
        WHEN u.Role = 'Traveler' THEN 'Traveler'
        WHEN u.Role = 'Operator' THEN 'Operator'
        WHEN u.Role = 'Provider' THEN 'Provider'
        ELSE 'Other'
    END AS UserType
FROM
    Person u
WHERE
    u.RDate >= DATEADD(MONTH, -12, GETDATE()) -- Tracks registrations for the last 12 months
GROUP BY
    MONTH(u.RDate), YEAR(u.RDate), 
    CASE
        WHEN u.Role = 'Traveler' THEN 'Traveler'
        WHEN u.Role = 'Operator' THEN 'Operator'
        WHEN u.Role = 'Provider' THEN 'Provider'
...
SELECT
    MONTH(b.Date) AS ActivityMonth,
    YEAR(b.Date) AS ActivityYear,
    COUNT(DISTINCT ba.TravelerID) AS ActiveTravelers,
    COUNT(DISTINCT t.OperatorID) AS ActiveOperators
FROM
    Booking b
JOIN
    BookingAttempt ba ON b.AttemptID = ba.AttemptID
JOIN
    Trip t ON ba.TripID = t.TripID  -- Join with the Trip table to get OperatorID
WHERE
    b.Date >= DATEADD(MONTH, -12, GETDATE()) -- Active users in the last 12 months
GROUP BY
    MONTH(b.Date), YEAR(b.Date)
	-------------------------------------------
	
	-----------------------------------------------
	
	CREATE VIEW view_PartnershipGrowth AS
SELECT
    MONTH(s.AssignmentDate) AS JoinMonth,
    YEAR(s.AssignmentDate) AS JoinYear,
    COUNT(DISTINCT s.ProviderID) AS NewHotels,
    COUNT(DISTINCT s.OperatorID) AS NewOperators
FROM
    ServiceAssignment s
WHERE
    s.AssignmentDate >= DATEADD(MONTH, -12, GETDATE()) -- Tracks new partnerships in the last 12 months
GROUP BY
    MONTH(s.AssignmentDate), YEAR(s.AssignmentDate)
ORDER BY
    JoinYear DESC, JoinMonth DESC;
	-----------------------------------------------
	ALTER TABLE TripDestinations
ADD  DateAdded DATE;
select * from TripDestinations
UPDATE TripDestinations
SET DateAdded = DATEADD(DAY, 
    (ROUND((RAND(CHECKSUM(NEWID())) * DATEDIFF(DAY, '2000-01-01', '2025-12-31')), 0)), -- Random days between 2000-01-01 and 2025-12-31
    '2000-01-01')

   CREATE VIEW view_RegionalExpansion AS
SELECT
    MONTH(td.DateAdded) AS AddedMonth,
    YEAR(td.DateAdded) AS AddedYear,
    COUNT(DISTINCT td.DestinationID) AS NewDestinations
FROM
    TripDestinations td
WHERE
    td.DateAdded >= DATEADD(MONTH, -12, GETDATE()) -- Tracks new destinations added in the last 12 months
GROUP BY
    MONTH(td.DateAdded), YEAR(td.DateAdded)
